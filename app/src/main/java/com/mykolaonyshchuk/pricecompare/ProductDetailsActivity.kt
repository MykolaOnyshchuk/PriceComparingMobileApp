...
class ProductDetailsActivity() : AppCompatActivity() {
    ...
    fun callback (dbHelperObj: DbHelper, productId: String) {
        ...
        if (detailedProduct.rearCamera != "") {
            rearCameraSpecs.text = detailedProduct.rearCamera
        } else {
            rearCameraSpecs.text = "—"
        }
        if (detailedProduct.frontCamera != "") {
            frontCameraSpecs.text = detailedProduct.frontCamera
        } else {
            frontCameraSpecs.text = "—"
        }
        if (detailedProduct.ScreenResolution != "") {
            screenResolutionSpecs.text = detailedProduct.ScreenResolution
        } else {
            screenResolutionSpecs.text = "—"
        }
        if (detailedProduct.ScreenSize != "") {
            screenSizeSpecs.text = detailedProduct.ScreenSize
        } else {
            screenSizeSpecs.text = "—"
        }
        if (detailedProduct.processor != "") {
            processorSpecs.text = detailedProduct.processor
        } else {
            processorSpecs.text = "—"
        }
        if (detailedProduct.internalMemory != "") {
            internalMemorySpecs.text = detailedProduct.internalMemory
        } else {
            internalMemorySpecs.text = "—"
        }
        if (detailedProduct.ram != "") {
            ramSpecs.text = detailedProduct.ram
        } else {
            ramSpecs.text = "—"
        }
        if (detailedProduct.battery != "") {
            batterySpecs.text = detailedProduct.battery
        } else {
            batterySpecs.text = "—"
        }
        Picasso.with(context).load(detailedProduct.imageUrl).placeholder(R.drawable.ic_baseline_smartphone_48).into(productImage)
    ...
    private fun buildComparingList(sortType: Int){
        ...
        for(item in priceComparingList){
            ...
            shop.text = item.shop
            shop.setTextColor(ContextCompat.getColor(this, R.color.black))
